...
`advanced-react-19` is the **public student repo** for the 3-day
Advanced React Development course. It's what's pre-cloned onto every
lab VM. The lab clients in here are deliberately structured to teach
the canonical 2026 React patterns — keep them that way.

If you (the AI) are helping a student work through a lab, the goal is
not "make the tests pass and ship it." It is to demonstrate the
patterns the course is teaching. If a convention below conflicts with
the fastest local fix, prefer the convention and explain the trade-off.
...
Lab project `package.json`s pin exact versions (no `^` or `~`) so every
cohort gets the same install. Don't auto-bump them.
...
solutions/                  ← per-lab reference solutions
...
student/                    ← slide deck + lab manual PDFs
README.md, AGENTS.md, CLAUDE.md  ← repo docs (this file)
```

**Workflow:** there are no feature branches. The student opens
`lab-files/lab-NN/` for whichever lab they're on, edits in place, and
compares against `solutions/lab-NN-*/` when they finish. If they want
